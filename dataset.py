...
class SpeedDating(Dataset):
    def __init__(self, group_id ='F1_Interaction_1', context_secs = 5, get_n_frames_per_sec = 10, prev_context_secs = 20, dir_path = "../data/speeddating/SpeedDating_labeled_BBox", utils = utils, transform=None):
        ...
        self.skip_secs = 5
        self.desired_feats = ['face_name', 'face_landmarks', 'keypoints', '3d_keypoints', 'gaze']
...
class RoomReader(Dataset):
    def __init__(self, group_id ='S03', context_secs = 5, get_n_frames_per_sec = 10, prev_context_secs = None, dir_path = "../data/roomreader/room_reader_corpus_db", utils = utils, transform=None):
        ...
        for sp in tqdm(self.speakers, position=1, leave=False): 
            
            # if 'T0' in sp:
            if sp[4] == 'T':
                self.teacher_feats = pd.read_csv(os.path.join(os.path.realpath(dir_path), "features/OpenFace_Features", sp)).iloc[:,5:].to_numpy()
            else:
                anot_csv_path = [s for s in anotlist if sp in s][0] #multiple engagment labels -- try 1 now
                ...
                feats_df = feats_df.iloc[::self.fps//self.get_n_frames_per_sec, :].to_numpy()
...
if __name__ == '__main__':
    ...
    for idx, batch in enumerate(tqdm(loader, position=1, leave=False)):
        pass
...
def get_meanstd(dataset):
    ...
    for i in iter(dataset):
        all_eng = np.concatenate((all_eng, i['eng'].flatten()))
    # print(all_eng.shape)
    return (np.nanmean(all_eng), np.nanstd(all_eng))